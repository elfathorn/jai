...
false;if(this.content.length>0){better=0;this.content.forEach(function(element,index,array){if(element[2].cost_f<array[better][2].cost_f)better=index})}return better===false?false:this.content[better]}};JAI.Map=function(min_x,max_x,min_y,max_y){this.min_x=min_x;this.max_x=max_x;this.min_y=min_y;this.max_y=max_y;this.length=0;this.points=new Object;for(var x=min_x;x<=max_x;x++)for(var y=min_y;y<=max_y;y++){this.points["x"+x+":y"+y]=new JAI.Point(x,y);this.length++}};JAI.Map.prototype={constructor:JAI.Map};
JAI.Astar=function(map){if(!(map instanceof JAI.Map))throw"new instance of JAI.Astar needs a JAI.Map";this.map=map;this.open_list=new JAI.List;this.close_list=new JAI.List};JAI.Astar.prototype={constructor:JAI.Astar};
